...
from flask import render_template, flash,\
                    redirect, url_for, request, g, jsonify, current_app
...
def index():
    ...
    return render_template(
        "index.html", title = 'Home Page', form = form, 
        posts = posts.items, 
...
def user(username):
    ...
    # posts = Post.query.filter_by(author = user).all()  # add by myself
...
def follow(username):
    ...
    db.session.commit()
    flash(_('You are followwing %(username)s', username = username))
    return redirect(url_for('main.user', username = username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash(_('You success to unfollow %(username)s.', username = username))
    return redirect(url_for('main.user', username = username))
